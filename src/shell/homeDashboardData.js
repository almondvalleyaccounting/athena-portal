...
//   - Recurring fees come from live_billing (status='active'), same as the
    ...
    (async () => {
      ...
      const [
        ...
        supabase
          .from('bm_task_schedule')
          .select(
            'id, bm_deadline, entity:entities!bm_task_schedule_entity_id_fkey(id, name), owner:staff_profiles!bm_task_schedule_assignee_id_fkey(name)',
          )
          .ilike('bm_task_name', CH_FILING)
          .eq('state', 'planned')
        ...
        supabase
          .from('live_billing')
          .select('monthly_net, monthly_gross, created_at, needs_review')
          .eq('status', 'active'),
        supabase
          .from('quotes')
          .select(
      ...
      const mrrNet = billing.reduce((s, r) => s + (r.monthly_net || 0), 0);
      const committedRows = billing.filter((r) => (r.created_at || '') >= monthStartIso);
      const committedThisMonth = committedRows.reduce((s, r) => s + (r.monthly_net || 0), 0);
      const billingNeedsReview = billing.filter((r) => r.needs_review).length;
      ...
      setState({
        ...
        data: {
          ...
          sa: { count: saCount, year: jan.year, runRate: saRunRate },
          overdueWork: { total: overdueTotal, byService: overdueByService },
          money: {
            mrrNet,
            activeBillingCount: billing.length,
            committedThisMonth,
            committedCount: committedRows.length,
            needsReview: billingNeedsReview,
          },
          quotes: {
            pendingApproval,
            sent: sentQuotes,
            sentAnnual: sentQuotes.reduce((s, q) => s + (q.annual_total || 0), 0),
