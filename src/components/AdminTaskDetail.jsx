...
import { Receipt, Flame, Send, ExternalLink, X } from 'lucide-react';
...
/*
  ...
  It has no chrome of its own — no page padding, no back button, no title bar —
  because it is rendered in two places and they frame it differently: the drawer
  below slides it in over the admin list, and AdminTaskDetailPage puts it on its
  own screen for a bookmarked /planner/tasks/:id. The body is the same either
  way, which is the point of it living here rather than in both.

  `onChanged` fires after every write so whatever is behind the drawer can
  refresh. A task edited here changes its row on the list — the title, the
  deadline, the pipeline step, the comment count — and a list that keeps showing
  the old values until the next reload is worse than no drawer at all.
*/
export default function AdminTaskDetail({ taskId, onChanged }) {
  ...
  // just by opening it.
  const isBillableService = (sid) => !!sid && serviceOptions.some((o) => o.id === sid);
  ...
  const save = async () => {
    ...
    const { error: e } = await supabase.from('admin_tasks').update(patch).eq('id', taskId);
  ...
  const addNote = async () => {
    ...
    const { data, error: e } = await supabase.from('admin_task_notes')
      .insert({ task_id: taskId, author_id: profile?.id || null, kind: 'note', body }).select('*').single();
  ...
  const setStage = async (stage) => {
    const { error: e } = await supabase.from('admin_tasks').update({ stage }).eq('id', taskId);
    if (e) { setError(e.message); return; }
    load();
    onChanged?.();
  };
  ...
  const addBill = async () => {
    ...
    const { error: ue } = await supabase.from('admin_tasks')
      .update({ billable: true, billing_item_id: b.id, stage: 'bill_hold', service_id: form.service_id }).eq('id', taskId);
  ...
  return (
    <div style={{ fontFamily: font }}>
      {/* Who it is for, and what raised it. The source chip lives here rather
          than in either wrapper's title bar, so the drawer and the page show
          the same thing. */}
      {(entity || task.source) && (
...
/*
  The same detail, slid in from the right over the list you opened it from.

  A whole-screen navigation for one task cost the list: its scroll position, its
  filters, which sections were open, and the sense of where you were in a run of
  twenty. The drawer keeps all of that behind it — the point of working through
  a list is the list.

  Escape closes it, so does the backdrop, and so does the browser Back button:
  the caller keeps the open task in the query string, which makes the drawer a
  history entry rather than something Back escapes by leaving the page entirely.
*/
export function AdminTaskDrawer({ taskId, onClose, onChanged }) {
  // Esc, and no scrolling the list underneath while it is open — a backdrop you
  // can scroll through reads as a page that half-responds.
  //
  // The lock goes on <main>, not on <body>: AppShell is h-screen with
  // overflow-hidden and hands the single scrolling viewport to <main>, so
  // body already has nothing to lock. Falls back to body if that ever changes.
  useEffect(() => {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', zIndex: 1000, animation: 'atd-fade 140ms ease-out' }}
    >
      <style>{`
        @keyframes atd-fade { from { opacity: 0; } to { opacity: 1; } }
        @keyframes atd-slide { from { transform: translateX(100%); } to { transform: translateX(0); } }
      `}</style>
      <div
        ...
        style={{
          position: 'fixed', top: 0, right: 0, bottom: 0, width: 620, maxWidth: '96vw',
          background: '#f8fafc', boxShadow: '-16px 0 48px rgba(15,23,42,0.18)',
          display: 'flex', flexDirection: 'column', fontFamily: font,
          animation: 'atd-slide 180ms cubic-bezier(0.32, 0.72, 0, 1)',
        }}
      >
        <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px 28px' }}>
          {/* Remounted per task, so opening a second row from the list cannot
              show the first one's form while the new task loads. */}
          <AdminTaskDetail key={taskId} taskId={taskId} onChanged={onChanged} />
        </div>
      </div>
    </div>
  );
