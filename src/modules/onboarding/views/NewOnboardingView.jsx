...
const input = {
  padding: '8px 12px', fontSize: 14, fontFamily: font, background: '#fff',
    ...
    <div style={{ padding: '24px 28px', fontFamily: font }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              ...
              {!entity && (
                <button
                  ...
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', color: '#0e7fe0', fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: font, padding: 0 }}
                >
            ...
            {entity ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 600, color: '#0f172a' }}>{entity.name}</div>
                  <div style={{ fontSize: 13, color: '#94a3b8' }}>{entity.entity_status}</div>
                </div>
                <button onClick={() => { setEntity(null); setPreview(null); setExisting([]); }} style={{ background: 'none', border: 'none', color: '#0e7fe0', fontSize: 13, cursor: 'pointer', fontFamily: font }}>
                  change
                </button>
              </div>
            ) : (
                ...
                <div style={{ maxHeight: 220, overflowY: 'auto', marginTop: 8 }}>
                  {entities.map((e) => (
                    <div
                      key={e.id}
                      onClick={() => setEntity(e)}
                      style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 14.5, color: '#0f172a', display: 'flex', justifyContent: 'space-between' }}
                      onMouseEnter={(ev) => (ev.currentTarget.style.background = '#f1f5f9')}
                      onMouseLeave={(ev) => (ev.currentTarget.style.background = 'transparent')}
                    >
                      <span>{e.name}</span>
                      <span style={{ fontSize: 12, color: '#94a3b8' }}>{e.entity_status}</span>
                    </div>
                  ))}
                </div>
              </>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {templates.map((t) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{t.steps.length} steps</div>
                </div>
              ))}
            </div>
          </div>
              ...
              <div>
                ...
                {referredBy ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14 }}>
                    ...
                    <button onClick={() => { setReferredBy(null); setRefSearch(''); }} style={{ background: 'none', border: 'none', color: '#0e7fe0', fontSize: 13, cursor: 'pointer', fontFamily: font }}>
                      change
                    </button>
                  </div>
                ) : (
        ...
        <div style={{ ...card, position: 'sticky', top: 16 }}>
          ...
          {!preview && <div style={{ fontSize: 14, color: '#94a3b8' }}>Pick a client and a template to preview the checklist.</div>}
          {preview && (
            <>
              <div style={{ fontSize: 13.5, marginBottom: 12, color: (preview.quote || preview.hasBilling) ? tones.success.fg : tones.warning.fg }}>
                {preview.quote
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 480, overflowY: 'auto' }}>
                {groups.map(([groupName, items]) => (
                  <div key={groupName}>
                    ...
                    {items.map((r) => (
                      <div key={r.templateStep.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 14, color: r.initialStatus === 'na' ? '#94a3b8' : '#0f172a' }}>
