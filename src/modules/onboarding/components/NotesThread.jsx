...
export default function NotesThread({ onboardingId, notes, onAdded, maxHeight = 360, autoFocus = false }) {
  ...
  return (
    <div style={{ fontFamily: font }}>
      ...
      {error && <div style={{ color: tones.danger.fg, fontSize: 12, marginBottom: 8 }}>{error}</div>}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight, overflowY: 'auto' }}>
        {sorted.length === 0 && <div style={{ fontSize: 12.5, color: '#94a3b8' }}>No notes yet.</div>}
        {sorted.map((n) => (
          <div key={n.id} style={{ fontSize: 12.5 }}>
