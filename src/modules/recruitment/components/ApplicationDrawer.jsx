...
import CommsPanel from './drawer/CommsPanel';
import InterviewsPanel from './drawer/InterviewsPanel';
import OfferPanel from './drawer/OfferPanel';
import ContractPanel from './drawer/ContractPanel';
import InductionPanel from './drawer/InductionPanel';

const TABS = [
  { key: 'overview', label: 'Overview' },
  { key: 'comms', label: 'Comms', pii: true },
  { key: 'interviews', label: 'Interviews', pii: true },
  { key: 'offer', label: 'Offer', pii: true },
  { key: 'contract', label: 'Contract', pii: true },
  { key: 'induction', label: 'Induction', pii: true },
];

// The application drawer — everything about one applicant on one vacancy,
// across the hire lifecycle, tabbed. PII tabs need candidate data loaded;
// if the candidate record is restricted, they show a permission note.
export default function ApplicationDrawer({ app, vacancy, staffMap, staffList, profileId, onClose, onPatch }) {
    ...
    <div onClick={onClose} style={backdrop}>
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          ...
          {tab === 'comms' && c && <CommsPanel app={app} vacancyTitle={vacancy?.title} profileId={profileId} />}
          {tab === 'interviews' && c && <InterviewsPanel app={app} vacancyTitle={vacancy?.title} staffList={staffList} staffMap={staffMap} profileId={profileId} />}
          {tab === 'offer' && c && <OfferPanel app={app} profileId={profileId} />}
          {tab === 'contract' && c && <ContractPanel app={app} profileId={profileId} />}
          {tab === 'induction' && c && <InductionPanel app={app} staffMap={staffMap} profileId={profileId} />}
        </div>
      </div>
    </div>
  );
...
function OverviewTab({ app, c, staffMap, staffList, profileId, onPatch }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
        {STAGES.map((s) => <StageBtn key={s.key} s={s} active={app.stage === s.key} onClick={() => onPatch({ stage: s.key })} />)}
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {EXIT_STAGES.map((s) => <StageBtn key={s.key} s={s} active={app.stage === s.key} onClick={() => onPatch({ stage: s.key })} />)}
      </div>
