...
export default function InterviewsView() {
  ...
  return (
    <div style={{ margin: '0 auto', fontFamily: font }}>
      ...
      <p style={{ fontSize: 14, color: '#64748b', margin: '0 0 16px' }}>Upcoming interviews across all open vacancies.</p>

      {error && <div style={{ fontSize: 14, color: '#b91c1c', marginBottom: 12 }}>{error}</div>}
      {items === null && <div style={{ ...card, padding: 18, textAlign: 'center', fontSize: 14, color: '#94a3b8' }}>Loading…</div>}
      {items !== null && items.length === 0 && (
        <div style={{ ...card, padding: '30px 18px', textAlign: 'center', fontSize: 14.5, color: '#94a3b8' }}>No interviews scheduled.</div>
      )}
          ...
          return (
            <div key={iv.id} onClick={() => appl?.vacancy_id && navigate(`/recruitment/${appl.vacancy_id}`)}
              ...
              <div style={{ textAlign: 'center', minWidth: 64 }}>
                <div style={{ fontSize: 13.5, fontWeight: 700, color: '#0e7fe0' }}>
                  {iv.scheduled_at ? fmtNoteTime(iv.scheduled_at) : 'TBC'}
                </div>
                <div style={{ fontSize: 12, color: '#94a3b8' }}>{iv.duration_mins}m</div>
              </div>
