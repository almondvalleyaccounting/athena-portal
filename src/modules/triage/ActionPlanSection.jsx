  ...
  return (
    <div style={{ marginTop: 18 }}>
          ...
          {menuOpen && (
            <>
              ...
              <div style={{
                ...
                {templates.length === 0 && (
                  <div style={{ fontSize: 13, color: '#94a3b8', padding: '8px 10px' }}>No active templates.</div>
                )}
                {templates.map((t) => (
      ...
      {actions.length === 0 && !c.next_action && (
        <div style={{ fontSize: 13.5, color: '#94a3b8', marginBottom: 8 }}>
        ...
        return (
          <div key={a.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              ...
              <span style={{
                fontSize: 13.5, fontWeight: 600, color: '#0f172a', flex: 1, minWidth: 0,
                textDecoration: a.status === 'cancelled' ? 'line-through' : 'none',
              }}>
                {a.title}
              </span>
              <button onClick={() => cycleStatus(a)} title="Click to change status" style={{
                fontSize: 11.5, fontWeight: 700, padding: '2px 8px', borderRadius: 999, cursor: 'pointer',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, paddingLeft: 20 }}>
              {open ? (
                <>
                  ...
                  {overdue && <span style={{ fontSize: 12, fontWeight: 600, color: '#dc2626' }}>overdue</span>}
                </>
              ) : (
      ...
      <div style={{ border: '1px dashed #e2e8f0', borderRadius: 8, padding: '8px 9px', marginTop: 8 }}>
        <label style={{ ...fieldLabel, fontSize: 12, color: '#94a3b8' }}>Add an action</label>
