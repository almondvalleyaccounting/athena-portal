-- ============================================================
-- Fix: admin_tasks_confirm_from_bm() has crashed on every run since
-- 13/07/2026, when the first bm_code task with field='ch_personal_code'
-- appeared (ch-codes Stage 5 "add to BM" to-do). The function does
--   execute format('select %I from entities …', field)
-- and entities has no ch_personal_code column (personal codes live on
-- people). The error was swallowed by both callers (page load ignores the
-- rpc error; bmClients.js wraps it in try/catch), so BM auto-confirm has
-- been silently dead — the one open ch_auth_code task never confirmed.
--
-- 1. admin_tasks_confirm_from_bm now only checks fields that actually
--    exist as columns on entities; anything else is skipped, not fatal.
-- 2. reconcile_ch_codes (the per-import CH personal-code reconciler, which
--    already receives BM's code per client) now also confirms matching
--    ch_personal_code bm_code tasks — restoring the "auto-confirmed by the
--    BM import" behaviour recordCodeReceived promises. Same reopened_at
--    guard as sql/115: a reopened task stays open until completed again.
--
-- Applied as migration admin_tasks_confirm_crash_fix (17/07/2026).
-- ============================================================
