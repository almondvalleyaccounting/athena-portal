-- ============================================================
-- Reallocation "Complete" → verify against BM re-import, reopen if wrong.
--
-- allocation_changes already has a draft -> committed/discarded lifecycle
-- (sql/051) but nothing ever set 'committed', and nothing ever re-checked
-- it against BM. This adds:
--   1. reconcile_allocation_changes() — for every 'committed' draft, compare
--      the proposed fee earner against the live BM-inferred assignee
--      (v_inferred_allocations, sql/052). If BM disagrees, revert the draft
--      back to 'draft' so it reappears on the Admin Task List and Capacity
--      Planner. No evidence either way (no matching BM tasks) leaves it
--      alone — same "reconcile on reimport, never silently undo without
--      evidence" convention as reconcile_field_overrides (bm_field
--      overrides) and admin_tasks_confirm_from_bm.
--   2. Wires that into import_bm_tasks (the RPC that writes
--      bm_task_schedule, which v_inferred_allocations is derived from) so
--      it runs on every BM tasks import, same place bm_field overrides are
--      reconciled inside import_bm_clients.
-- ============================================================

create or replace function public.reconcile_allocation_changes()
returns int
language plpgsql
security definer
...
begin
  for d in
    select id, entity_id, canonical_service_id, proposed_fee_earner_id
    from allocation_changes
    where status = 'committed'
  loop
    select assignee_id into bm_assignee
    from v_inferred_allocations
    where entity_id = d.entity_id and canonical_service_id = d.canonical_service_id
    limit 1;

    -- Only reopen when BM shows a live, disagreeing assignee. Missing
    -- data (no matching BM tasks) is not evidence of failure.
    if bm_assignee is not null and bm_assignee is distinct from d.proposed_fee_earner_id then
      if not exists (
        select 1 from allocation_changes
        where entity_id = d.entity_id and canonical_service_id = d.canonical_service_id and status = 'draft'
      ) then
        update allocation_changes
        set status = 'draft', committed_at = null, committed_by = null
        where id = d.id;
        reverted := reverted + 1;
      end if;
    end if;
...
comment on function public.reconcile_allocation_changes() is
  'Reopens a "committed" allocation_changes draft (back to status=draft) when the latest BM import still shows a different assignee than proposed. Called from import_bm_tasks after every BM tasks import, and from the Admin Task List on page load.';

-- ── import_bm_tasks: call the reconciler at the end, alongside the
--    existing disappearance/completion sweep. Full function body required
--    for CREATE OR REPLACE — only change vs the live definition is the
--    counter_realloc_reverted variable, the reconcile call, and its count
...
declare
  ...
  counter_realloc_reverted int := 0;
