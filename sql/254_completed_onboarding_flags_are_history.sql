-- 254: A completed onboarding's service flags are history — and BM's LOE date
-- is only "stale Athena" when Athena has a step to be stale against.
--
-- Hashtag Rose surfaced the first: "awaiting registration" for VAT and PAYE it
-- has never had — and a waiting BrightPay mark downstream of the PAYE one —
-- with no fee, no BM scheduled work, no reference, no HMRC presence. The only
-- source was onboardings.service_conditions = [ct, paye, sa, vat, bookkeeping]
-- on an onboarding marked complete on 2026-07-12, the day of the tracker
-- import. The seeded VAT/PAYE steps were pending then (status_before_auto
-- shows it) — the old combined "Bookkeeping & VAT" meaning sql/102 left to be
-- unticked by hand. The flag rule exists for the Velvet Rogue case: an
-- onboarding IN FLIGHT waiting on a number. So flags now count only from
-- active / on-hold / issues onboardings. 66 flag-only "awaiting registration"
-- rows, all on completed onboardings, come off; the in-flight ones stay.
--
-- The second arrived with the first BM export to carry the engagement-letter
-- column (2026-08-21, 593 dates). 503 of those clients have no onboarding at
-- all — BM holding the date IS the record — yet loe_from_bm_only fired and
-- drowned the loose-end verdict (9 → 406). It now requires an onboarding
-- whose step nobody ticked.
--
-- Both statements are create-or-replace over the live definitions (sql/249's
-- per-tax view and sql/251's client roll-up) — same columns, so the dependent
-- views need no rebuild. The stale service_conditions rows themselves are left
-- untouched: they record what the import selected; the view is the layer that
-- interprets them.
...
comment on view v_onboarding_crosscheck_client is 'One row per active client. loe_signed accepts either record (Athena step ticked by a person, or BM signed date); loe_from_bm_only fires only when an onboarding exists whose step was never ticked. Reference-missing states count as awaiting_registration; billed_vat_not_registered flags a VAT product sold to a client with no VAT registration.';
