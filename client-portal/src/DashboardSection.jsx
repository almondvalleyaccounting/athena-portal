...
} from '@dash/overviewGrain.js';
import { money, moneyCompact, shortDate } from '@dash/dashboardData.js';
import { BucketChart, LineChart } from '@dash/DashboardCharts.jsx';
import {
  forecastByMonth, actualsByMonth, buildStatement, buildCashflow,
  totalRow, netRow, PL_ORDER, BS_ORDER,
} from '@dash/projectionEngine.js';

/*
  The client's own financial dashboard.

  Everything here is served by the portal-dashboard edge function, which decides
  what this login may see — the sections, the company, and the dates. This
  component asks for a grain and a basis and renders whatever comes back; it
  cannot ask for another company, another date range or another metric, because
  there is nothing in the request that could express any of those.

  The arithmetic is NOT reimplemented for the portal. The bucketing, the
  owner-cost adjustment and the projection engine are imported from
  src/modules/client-dashboard (see the @dash alias in vite.config.js), which is
  the same code the staff dashboard runs. A client and their accountant looking
  at the same month have to see the same number, and the only reliable way to
...
export default function DashboardSection({ onHasDashboards }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // A portal built before sql/238 simply has no dashboards. Not an error
        // the client should ever be shown.
        setGrants([]);
  ...
  const tabs = [
    { key: 'overview', label: 'Overview', on: sections.overview !== false },
    { key: 'pl', label: 'Profit & loss', on: !!sections.pl },
    { key: 'bs', label: 'Balance sheet', on: !!sections.balance },
    { key: 'projection', label: 'Projection', on: !!sections.projection && !!payload?.projection },
  ].filter((x) => x.on);
    ...
    <div style={{ marginTop: 30 }}>
      <div className="fade-up" style={{
        ...
        <div style={{ fontSize: 'clamp(19px, 4.2vw, 24px)', fontWeight: 700, margin: '5px 0 4px' }}>
          {payload?.company_name || grant?.entity_name || 'Your business'}
        </div>
        ...
        {grants.length > 1 && (
          <select
            value={entityId}
            onChange={(e) => { setEntityId(e.target.value); setPayload(null); }}
            style={{
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
        <Pills options={GRAINS} value={grain} onChange={setGrain} />
        <Pills
          options={[
      ...
      {error && (
        <div style={{ fontSize: 13.5, color: '#b91c1c', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '10px 14px' }}>
          {error}{' '}
          <button onClick={load} style={{ border: 'none', background: 'none', color: '#b91c1c', fontWeight: 700, cursor: 'pointer', textDecoration: 'underline' }}>
            Try again
          </button>
        </div>
      )}
      ...
      {payload && active === 'overview' && (
        <Overview payload={payload} grain={grain} basis={basis} view={view} />
      )}
      {payload && active === 'pl' && (
        <ProfitAndLoss payload={payload} grain={grain} basis={basis} view={view} />
      )}
...
function ProfitAndLoss({ payload, grain, basis, view }) {
  ...
  const line = (label, pick, bold) => ({
    label, bold, values: chartRows.map(pick),
  });
  const isU = view === 'underlying';
  const lines = [
  ...
  return (
    <Card pad={false}>
      ...
      <ScrollTable
        columns={chartRows.map((r) => r.label)}
        rows={lines}
        currency={currency}
      />
    </Card>
...
function Projection({ payload, grain, basis }) {
  ...
  const income = totalRow(pl.rows, 'inc', (r) => r.kind === 'income');
  const net = netRow(pl.rows);
...
function Tile({ label, value, prev, currency, sub, goodWhenDown }) {
  ...
  return (
    <div className="fade-up" style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 14, padding: '13px 15px' }}>
